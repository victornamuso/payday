module Payday
  ...
  class Invoice
    ...
    attr_accessor :invoice_number, :bill_to, :ship_to, :notes, :line_items, :shipping_rate, :shipping_description,
      :tax_rate, :tax_description, :due_at, :paid_at, :refunded_at, :currency, :invoice_details, :invoice_date,:po_number,:terms,:company_details,:company_name,:tax

    def initialize(options =  {})
      self.invoice_number = options[:invoice_number] || nil
      self.bill_to = options[:bill_to] || nil
      self.po_number = options[:po_number] || nil
    ...
    def tax_rate=(value)
      @tax_rate =BigDecimal(value.present? ? value.to_s : 0)
    end

    # Shipping rate
    def shipping_rate=(value)
      @shipping_rate = BigDecimal(value.present? ? value.to_s : 0)
    end
  end
end
